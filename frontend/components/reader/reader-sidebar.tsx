...
export function ReaderSidebar({ chapter, nav, mangaSlug, mangaTitle, onClose }: Props) {
    ...
    <aside className="w-[70vw] md:w-[380px] min-w-0 md:min-w-[380px] h-full flex flex-col bg-surface border-l border-default">
      ...
      <div className="px-4 pt-3 pb-1">
        <Link
          ...
          className="text-primary font-semibold text-sm hover:text-accent transition-colors line-clamp-1"
        >
          {mangaTitle}
        </Link>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3">
        ...
        <select
          ...
          className="flex-1 h-9 bg-elevated border border-default rounded-md px-3 text-sm text-primary focus:outline-none focus:ring-1 focus:ring-accent appearance-none cursor-pointer text-center"
        >
          ...
          {sortedChapters.length === 0 && (
            <option value={chapter.id}>Ch {chapter.number}</option>
          )}
        </select>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        <div className="text-xs text-warning bg-warning/10 rounded-md px-3 py-2 mb-3">
          Note: Please take a moment to{' '}
          <span className="underline cursor-pointer">read the comment rules</span>{' '}
          before posting.
        </div>
