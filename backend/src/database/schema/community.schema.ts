...
export const readingHistory = pgTable('reading_history', {
  ...
  uniqueIndex('reading_history_user_manga_idx').on(table.userId, table.mangaId),
...
export const stickers = pgTable('stickers', {
  id: serial('id').primaryKey(),
  stickerSetId: integer('sticker_set_id').notNull().references(() => stickerSets.id, { onDelete: 'cascade' }),
  name: varchar('name', { length: 100 }).notNull(),
