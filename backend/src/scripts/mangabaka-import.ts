...
const SOURCE_TYPE_MAP: Record<string, string> = {
  my_anime_list: 'mal', anilist: 'anilist', kitsu: 'kitsu',
  manga_updates: 'mu', anime_planet: 'anime-planet',
  shikimori: 'shikimori', anime_news_network: 'ann',
};
...
function normalizeSeries(raw: any) {
  ...
  const genreNames = raw.genres_v2?.length
    ? raw.genres_v2.filter((g: any) => g.group === 'genre').map((g: any) => g.name)
    : (raw.genres ?? []).map((g: string) => g.replace(/_/g, ' '));
  const themeNames = raw.tags_v2?.length
    ? raw.tags_v2.filter((t: any) => t.is_genre === false).map((t: any) => t.name)
    : (raw.tags ?? []);
  ...
  if (raw.source) {
    for (const [key, data] of Object.entries(raw.source as Record<string, any>)) {
      const type = SOURCE_TYPE_MAP[key];
      if (type && data?.id != null) links.push({ type, externalId: String(data.id) });
    }
  }
  if (raw.links) {
...
async function main() {
  ...
  for (let page = PAGE_FROM; page <= PAGE_TO; page++) {
    const data = await api<any>(`/v1/series/search?q=${encodeURIComponent(QUERY)}&page=${page}&limit=${LIMIT}`);
    ...
    console.log(`  Page ${page}/${totalPages}: +${results.length} results, ${mangaList.length} unique total`);

    if (!data.pagination?.next) { console.log('  No more pages.'); break; }
  }

  console.log(`\nFound ${mangaList.length} manga to import\n`);

  if (DRY_RUN) {
    mangaList.forEach((m, i) => console.log(`  ${i + 1}. [${m.id}] ${m.title} (${m.type ?? 'manga'}, ${m.status ?? '?'})`));
    console.log('\n(dry run)');
    await sqlClient.end();
    return;
  }
