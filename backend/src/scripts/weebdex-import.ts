...
async function importOneManga(
  ...
  const altTitles = raw.alt_titles
    ? (Object.values(raw.alt_titles).flat() as string[])
    : [];
  const coverUrl =
    rels.cover?.id && rels.cover?.ext
  ...
  return upsertManga({
    ...
    year: raw.year ?? null,
    genreNames: tags
      .filter((t: any) => t.group === 'genre')
      .map((t: any) => t.name),
    themeNames: tags
      .filter((t: any) => t.group === 'theme')
      .map((t: any) => t.name),
...
async function importChapters(
  ...
  while (hasMore) {
    const apiData = await api<any>(
      `/manga/${mangaExtId}/chapters?limit=100&page=${page}`,
    );
    const filtered =
      LANG === 'all'
        ? (apiData.data ?? [])
    ...
    for (const raw of filtered) {
      ...
      const [existingSrc] = await db
        ...
        .where(eq(schema.chapterSources.externalId, raw.id))
        .limit(1);
      if (existingSrc) continue;

      const num = parseFloat(raw.chapter ?? '0');
      const groups = (raw.relationships?.groups ?? []).filter(
        (g: any) => !!g.name,
      );
      ...
      if (groups.length) {
        const groupIds = await resolveByName(
          schema.groups,
          ...
          await db
            ...
            .values(
              images.map((img: any, idx: number) => ({
                ...
                pageNumber: idx + 1,
                order: idx + 1,
                width: img.dimensions?.[0] ?? null,
                height: img.dimensions?.[1] ?? null,
              })),
            )
            .onConflictDoNothing();
...
async function main() {
  ...
  for (let page = PAGE_FROM; page <= PAGE_TO; page++) {
    ...
    for (const m of data.data) {
      ...
      if (LANG === 'all' || langs.includes(LANG))
        mangaList.push({ id: m.id, title: m.title, langs });
    }
    console.log(
      `  Page ${page}: ${data.data.length} scanned, ${mangaList.length} matched`,
    );
  }
  ...
  if (DRY_RUN) {
    mangaList.forEach((m, i) =>
      console.log(`  ${i + 1}. ${m.title} [${m.langs.join(',')}]`),
    );
    console.log('\n(dry run)');
    await sqlClient.end();
    return;
  ...
  for (let i = 0; i < mangaList.length; i++) {
    ...
    try {
      ...
      const label = isNew ? '→' : '↻';
      console.log(
        `${tag} ${m.title} ${label} id:${mangaId}, +${r.chapters} ch, +${r.images} img`,
      );
    } catch (err: any) {
      failed++;
